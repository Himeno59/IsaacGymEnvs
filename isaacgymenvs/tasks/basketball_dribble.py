...
class BasketballDribble(VecTask):
    ...
    def _create_envs(self, num_envs, spacing, num_per_row):
        ...
        object_start_pose.p.x, object_start_pose.p.y, object_start_pose.p.z = 1.5, 0, 0.2
    ...
    def compute_reward(self, actions):
        ...
        self.rew_buf[:], self.reset_buf[:] = compute_dribble_reward(
            self.obs_buf,
            self.reset_dist,
            self.reset_buf,
            self.progress_buf,
...
def compute_dribble_reward(
    ...
    # target_height = 3.0
    target_height_reward = 1.0 - abs(1.0 - ball_z)**2
    ...
    reset = reset_buf
    reset = torch.where(abs(ball_x - 1.5) > 2.0, torch.ones_like(reset_buf), reset_buf) # x
